...
- Rebuild the deck after every slide or theme change so `slides/output/teaching-proof-ai-re.pdf` stays current.
...
- Update `archive/teaching-proof-ai-re.pdf` only when the archival copy is intentionally being refreshed.
