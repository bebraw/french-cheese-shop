...
function findContainingPanel(element, shapes) {
  const filledShapes = shapes.filter((shape) => {
    const fill = shape.meta.options && shape.meta.options.fill;
    return (
      Boolean(shape.box && fill && fill.color) &&
      Number(fill.transparency || 0) < 100
    );
  });

  const overlapping = filledShapes
    .map((shape) => ({
      contains: boxContains(shape.box, element.box, 0.02),
      density: overlapArea(shape.box, element.box) / Math.max(shape.box.w * shape.box.h, Number.EPSILON),
      overlap: overlapArea(shape.box, element.box),
      shape
    }))
    .filter((entry) => entry.overlap > 0)
    .sort((left, right) => {
      if (right.contains !== left.contains && Math.abs(right.density - left.density) < 0.05) {
        return Number(right.contains) - Number(left.contains);
      }
  ...
  return overlapping[0] ? overlapping[0].shape : null;
}
